...
const StatsCard: React.FC<StatsCardProps> = ({ title, value }) => {
  return (
    <div className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-purple-50 to-transparent p-3 transition-all duration-300 hover:from-purple-100">
    <div className="flex items-center justify-center gap-3">
      ...
      <span className="font-bold text-lg text-gray-700">{value.toLocaleString()}</span>
    </div>
    <p className="text-sm text-center text-gray-500 mt-1">{title}</p>
  </div>
  )
}
